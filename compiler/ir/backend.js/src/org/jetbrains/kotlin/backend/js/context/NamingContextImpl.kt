...
class NamingContextImpl(
    ...
    private fun buildQualifiedExpression(descriptor: DeclarationDescriptor): JsNameRef {
        if (descriptor is ClassDescriptor) {
            ...
            if (KotlinBuiltIns.isAny(classDescriptor)) {
                ...
                return pureFqn("Long", pureFqn("Kotlin"))
            }
            else if (KotlinBuiltIns.isChar(type)) {
                return pureFqn("BoxedChar", pureFqn("Kotlin"))
            }
            ...
            else if (classDescriptor.defaultType.isNotNullThrowable()) {
        ...
        if (suggested == null) {
            val module = descriptor.module
            return if (module != currentModule) pureFqn(getModuleInnerName(module)) else pureFqn("_")
        }
    ...
    private fun getSuggestedName(descriptor: DeclarationDescriptor): String {
        val (suggestedName, container) = when (descriptor) {
            is PropertyGetterDescriptor -> Pair("get_${descriptor.name}", descriptor.correspondingProperty.containingDeclaration)
            is PropertySetterDescriptor -> Pair("set_${descriptor.name}", descriptor.correspondingProperty.containingDeclaration)
            is ConstructorDescriptor -> return getSuggestedName(descriptor.containingDeclaration) + "_init"
            else -> {
