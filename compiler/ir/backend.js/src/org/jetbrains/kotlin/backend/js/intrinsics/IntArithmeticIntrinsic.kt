...
import org.jetbrains.kotlin.backend.js.expression.TypedExpression
...
object IntArithmeticIntrinsic : BinaryOperationIntrinsic() {
    ...
    override fun apply(context: IrTranslationContext, call: IrCall, first: TypedExpression, second: TypedExpression): TypedExpression {
        ...
        val result = JsBinaryOperation(JsBinaryOperator.BIT_OR, JsBinaryOperation(operator, first.expression, second.expression),
                                       JsIntLiteral(0))
        return TypedExpression(result, call.type)
    }
}