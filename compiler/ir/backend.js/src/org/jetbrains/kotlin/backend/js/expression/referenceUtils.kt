...
private fun getPrototypeIfNecessary(descriptor: ClassDescriptor, reference: JsExpression): JsExpression {
    return if (DescriptorUtils.isObject(descriptor) || DescriptorUtils.isEnumEntry(descriptor)) {
        val getPrototypeRef = pureFqn("getPrototypeOf", pureFqn("Object"))
        val getPrototypeInvocation = JsInvocation(getPrototypeRef, reference)
        getPrototypeInvocation.sideEffects = SideEffectKind.PURE
        pureFqn("constructor", getPrototypeInvocation)
    }
    else {
        reference
    }
